...
assert provisioner.services.pd-pki-workflow.liveHardware.managedProvisioningInputs.enable;
assert !signer.services.pd-pki-workflow.liveHardware.managedProvisioningInputs.enable;
assert provisionerApiEnvironment.PD_PKI_WEB_HIDE_CURSOR == "1";
assert provisionerApiEnvironment.PD_PKI_WEB_THEME == "dark";
assert builtins.elem "pd-pki-live-provisioner-input-sync.service" provisioner.systemd.services.pd-pki-api.after;
assert builtins.elem "pd-pki-live-provisioner-input-sync.service" provisioner.systemd.services.pd-pki-api.wants;
assert provisioner.systemd.timers.pd-pki-live-provisioner-input-sync.wantedBy == [ "timers.target" ];
assert !(builtins.hasAttr "pd-pki-live-provisioner-input-sync" signer.systemd.timers);
