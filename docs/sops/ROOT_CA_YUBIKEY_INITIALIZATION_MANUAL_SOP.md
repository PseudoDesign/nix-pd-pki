# Root CA SOP: Manual YubiKey Initialization Fallback

## Purpose

This fallback SOP preserves the manual, step-by-step procedure for initializing
a dedicated YubiKey as the root CA signing token on an offline NixOS
workstation.

Use [`ROOT_CA_YUBIKEY_INITIALIZATION_SOP.md`](ROOT_CA_YUBIKEY_INITIALIZATION_SOP.md)
as the primary procedure. Keep this manual version for investigation,
recovery, or environments where `pd-pki-signing-tools init-root-yubikey`
cannot be used.
