...
let
  ...
  initScript = pkgs.writeShellScript "pd-pki-openvpn-client-leaf-init" ''
    ...
    jq -n \
      --arg identity ${lib.escapeShellArg cfg.request.commonName} \
...
in
{
  ...
  options.services.pd-pki.roles.openvpnClientLeaf = {
    ...
    request = lib.mkOption {
      default = { };
      description = ''
        Declarative OpenVPN client request contract. These settings define the exported request
        bundle basename, certificate identity, requested profile, and requested lifetime that
        pd-pki validates before importing a signed client certificate.
      '';
      type = lib.types.submodule {
        options = {
          basename = lib.mkOption {
            type = lib.types.str;
            default = runtimeDefaults.client.basename;
            description = ''
              Public artifact label for the OpenVPN client request bundle. This controls the CSR
          ...
          commonName = lib.mkOption {
            ...
            description = ''
              Common name requested for the OpenVPN client certificate.
            '';
          };

          extraSubjectAltNames = lib.mkOption {
            type = lib.types.listOf lib.types.str;
            default = runtimeDefaults.client.extraSubjectAltNames;
            description = ''
          ...
          subjectAltNames = lib.mkOption {
            ...
            description = ''
              Subject alternative names requested for the OpenVPN client certificate. By default
          ...
          requestedProfile = lib.mkOption {
            type = lib.types.str;
            default = runtimeDefaults.client.profile;
            description = ''
              Extended key usage profile requested from the signer for the client certificate.
            '';
          };

          requestedDays = lib.mkOption {
            type = lib.types.ints.positive;
            default = builtins.fromJSON runtimeDefaults.client.days;
            description = ''
              Requested certificate lifetime for the client certificate. Signer policy may shorten
