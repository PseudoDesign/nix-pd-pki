...
let
  ...
  initScript = pkgs.writeShellScript "pd-pki-openvpn-client-leaf-init" ''
    ...
    sign_tls_certificate \
      "$leaf_workdir" \
      ${lib.escapeShellArg runtimeDefaults.client.basename} \
      "$request_workdir/client.csr.pem" \
      ${lib.escapeShellArg sanSpec} \
      ${lib.escapeShellArg runtimeDefaults.client.profile} \
      ${lib.escapeShellArg runtimeDefaults.client.serial} \
      ${lib.escapeShellArg runtimeDefaults.client.days} \
      "$intermediate_key" \
      "$intermediate_cert" \
      "$root_cert"
    ...
    cp "$leaf_workdir/client.cert.pem" "$cert_path"
...
in
{
  ...
  options.services.pd-pki.roles.openvpnClientLeaf = {
    ...
    generateRuntimeSecrets = lib.mkOption {
      ...
      default = true;
      description = ''
        Whether to generate simulated runtime OpenVPN client material under the mutable state directories.
      '';
    };
