  ...
  initScript = pkgs.writeShellScript "pd-pki-openvpn-client-leaf-init" ''
    ...
    if [ -n "$request_generation_key_path" ] && [ ! -f "$candidate_csr_path" ]; then
      ...
      if [ "$allow_local_key_generation" != "1" ]; then
        printf '%s\n' "Refusing to generate a client key locally; provide keySourcePath or seed the runtime state first" >&2
        exit 1
      fi
      generate_tls_request \
        "$request_material_dir" \
      ...
      printf '%s\n' "Refusing to keep a client CSR without a matching private key in $state_dir" >&2
      exit 1
    fi
...
in
{
  ...
  options.services.pd-pki.roles.openvpnClientLeaf = {
    ...
    allowLocalKeyGeneration = lib.mkOption {
      ...
      default = true;
      description = ''
        Whether the module may generate a runtime client private key locally when no key is
