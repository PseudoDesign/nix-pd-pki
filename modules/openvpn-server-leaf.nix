...
let
  ...
  initScript = pkgs.writeShellScript "pd-pki-openvpn-server-leaf-init" ''
    ...
    if [ -f "$candidate_key_path" ] && [ -f "$candidate_csr_path" ]; then
      if ! private_key_matches_csr "$candidate_key_path" "$candidate_csr_path" || ! validate_tls_csr_matches_request "$candidate_csr_path" "$request_path"; then
    ...
    if [ -n "$request_generation_key_path" ] && [ ! -f "$candidate_csr_path" ]; then
      ...
      if [ "$allow_local_key_generation" != "1" ]; then
        printf '%s\n' "Refusing to generate a server key locally; provide keySourcePath or seed the runtime state first" >&2
        exit 1
      fi
      generate_tls_request \
        "$request_material_dir" \
      ...
      printf '%s\n' "Refusing to keep a server CSR without a matching private key in $state_dir" >&2
      exit 1
    fi
...
in
{
  ...
  options.services.pd-pki.roles.openvpnServerLeaf = {
    ...
    keyAlgorithm = lib.mkOption {
      ...
      default = runtimeDefaults.server.keyAlgorithm;
      description = ''
        Private key algorithm used when generating the runtime server CSR locally.
      '';
    };

    rsaKeyBits = lib.mkOption {
      type = lib.types.ints.positive;
      default = runtimeDefaults.server.rsaKeyBits;
      description = ''
        RSA modulus size used for locally generated runtime server keys when `keyAlgorithm` is
        `rsa`.
      '';
    };
  ...
  config = lib.mkIf (cfg.enable && cfg.generateRuntimeSecrets) {
    ...
    systemd.services.pd-pki-openvpn-server-leaf-refresh = lib.mkIf (refreshSourcePaths != [ ] && cfg.refreshInterval != null) {
    ...
    systemd.timers.pd-pki-openvpn-server-leaf-refresh = lib.mkIf (refreshSourcePaths != [ ] && cfg.refreshInterval != null) {
